...
def register(app: Client):
    ...
    async def deposit_command(client: Client, message: Message):
        ...
        if len(args) < 2:
            await message.reply_text(loc.t("economy.deposit_prompt", lang))
            return
        try:
            amount = int(args[1])
        except ValueError:
        ...
        if message._services:
            ...
            if eco_svc:
                result = await eco_svc.deposit(user.id, amount)
                if result["success"]:
                    text = f"{loc.t('economy.deposit_success', lang, amount=format_number(amount))}\n💰 Wallet: {format_number(result['wallet'])}\n🏦 Bank: {format_number(result['bank'])}"
                else:
                    text = loc.t("economy.deposit_failed", lang)
                await message.reply_text(text)
                return
        await message.reply_text(loc.t("error.general", lang))

    @app.on_callback_query(filters.regex("^deposit$"))
    async def deposit_callback(client: Client, callback_query: CallbackQuery):
        lang = "en"
        await callback_query.message.edit_text(
            loc.t("economy.deposit_prompt", lang),
